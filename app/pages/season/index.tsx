...
export const SeasonPage: React.FC<Props> = ({ className, seasonData }) => {
    return (
        <div className="flex flex-col gap-6 text-white">
            <h1 className="text-4xl md:text-6xl text-white text-center font-semibold">{seasonData.titleRu}</h1>
            ...
            <div className='flex flex-col  gap-9'>
                {seasonData.season_stories.map((season_story, index) => (
                    <SeasonStory index={index} key={index} text={season_story.textRu} title={season_story.titleRu} image={season_story.image.url} />
